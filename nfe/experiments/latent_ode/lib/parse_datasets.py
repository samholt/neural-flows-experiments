...
def sine(device, trajectories_to_sample=100):
    t_end = 20.0
    t_nsamples = 200
    t_begin = t_end / t_nsamples
    t = torch.linspace(t_begin, t_end,
...
def parse_datasets(args, device):
    ...
    if dataset_name == 'physionet':
        ...
        train_data, val_data, test_data = utils.split_train_val_test(
            total_dataset)
    ...
    if dataset_name == 'activity':
        ...
        dataset_obj = PersonActivity(
            DATA_DIR / 'activity' if LOCAL else DATA_DIR, download=True, n_samples=n_samples, device=device)

        train_data, test_data = model_selection.train_test_split(
            dataset_obj, train_size=0.8, shuffle=False)
        train_data, val_data = model_selection.train_test_split(
            dataset_obj, train_size=0.75, shuffle=False)
    ...
    if dataset_name == 'sine':
        ...
        batch_size = 128
        dltrain = DataLoader(train_trajectories, batch_size=batch_size, shuffle=True,
                             collate_fn=lambda batch: basic_collate_fn(batch, t, data_type='train', args=args))
        dlval = DataLoader(val_trajectories, batch_size=batch_size, shuffle=False,
                           collate_fn=lambda batch: basic_collate_fn(batch, t, data_type='test', args=args))
        dltest = DataLoader(test_trajectories, batch_size=batch_size, shuffle=False,
                            collate_fn=lambda batch: basic_collate_fn(batch, t, data_type='test', args=args))
